export const LOGINREQUIRED = 'error.login.required';
export const COMMONSERVERERROR = 'error.server.error';

export const errorRole = (customCode: string): string => {
  return `error.role.${customCode}`;
};

export const errorRolesUsers = (customCode: string): string => {
  return `error.rolesusers.${customCode}`;
};

export const errorUser = (customCode: string): string => {
  return `error.user.${customCode}`;
};

export const errorAuth = (customCode: string): string => {
  return `error.auth.${customCode}`;
};